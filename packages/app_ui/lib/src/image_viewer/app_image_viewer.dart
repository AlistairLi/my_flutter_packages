...
import 'package:app_ui/src/image_viewer/dots_indicator_style.dart';
...
///  · 缩放/点击
///  · 指示器
class AppImageViewer<T> extends StatefulWidget {
  /// 组件宽度
  final double? width;
  ...
  /// 图片点击回调
  final void Function(int index, T item)? onImageTap;
...
class _AppImageViewerState<T> extends State<AppImageViewer<T>> {
  ...
  Widget _buildNormalImage(ImageProvider imageProvider, int index, T item) {
    ...
    return GestureDetector(
      onTap: () => widget.onImageTap?.call(index, item),
      child: imageWidget,
    );
  }
  ...
  Widget _buildIndicator() {
    ...
    if (widget.isPage &&
      ...
      indicator = Container(
        ...
        margin: EdgeInsets.only(top: statusBarHeightPixel),
        child: switch (widget.indicatorType) {
          IndicatorType.dots => _buildDotsIndicator(),
          IndicatorType.number => _buildNumberIndicator(),
        },
      );
    } else {
